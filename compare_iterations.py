#!/usr/bin/env python3
"""Generate comparison plot: BLIS-Evolved Iter16 vs Iter24 vs Iter26 vs Iter29 vs Iter36."""
...
def main():
    """Generate iter16 vs iter24 vs iter26 vs iter29 vs iter36 comparison figure.

    Note: This script requires results_iter16/, results_iter24/, results_iter26/,
    results_iter29/, and results_iter36/ directories with error_records.csv files.
    Generate them by running:
        python -m experiment.run --adapters blis-evolved --blis-evolved-iteration {16,24,26,29,36}
    """
    ...
    # Load iter36 results
    print("Loading iter36 results...")
    try:
        iter36_errors = load_error_data("results_iter36/error_records.csv")
    except FileNotFoundError:
        print("✗ results_iter36/error_records.csv not found")
        print("  Run: python -m experiment.run --adapters blis-evolved --blis-evolved-iteration 36")
        return 1
    # Rename simulator to distinguish from iter16/iter24/iter26/iter29
    iter36_errors["simulator"] = iter36_errors["simulator"].replace(
        "blis-evolved", "blis-evolved-iter36"
    )
    ...
    iter36_errors = iter36_errors[iter36_errors["simulator"] == "blis-evolved-iter36"]
    ...
    print(f"  Iter36: {len(iter36_errors)} records, {iter36_errors['experiment_folder'].nunique()} experiments")

    # Combine
    combined = pd.concat([iter16_errors, iter24_errors, iter26_errors, iter29_errors, iter36_errors], ignore_index=True)
    ...
    iter36_exps = set(combined[combined["simulator"] == "blis-evolved-iter36"]["experiment_folder"].unique())
    common_exps = iter16_exps & iter24_exps & iter26_exps & iter29_exps & iter36_exps

    print(f"\nExperiment overlap:")
    print(f"  Common experiments: {len(common_exps)}")
    print(f"  Iter16 only: {len(iter16_exps - iter24_exps - iter26_exps - iter29_exps - iter36_exps)}")
    print(f"  Iter24 only: {len(iter24_exps - iter16_exps - iter26_exps - iter29_exps - iter36_exps)}")
    print(f"  Iter26 only: {len(iter26_exps - iter16_exps - iter24_exps - iter29_exps - iter36_exps)}")
    print(f"  Iter29 only: {len(iter29_exps - iter16_exps - iter24_exps - iter26_exps - iter36_exps)}")
    print(f"  Iter36 only: {len(iter36_exps - iter16_exps - iter24_exps - iter26_exps - iter29_exps)}")
    ...
    output_path = "ITER16_vs_ITER24_vs_ITER26_vs_ITER29_vs_ITER36_comparison.pdf"
    ...
    figures_module.SIMULATOR_ORDER.extend(["blis-evolved-iter16", "blis-evolved-iter24", "blis-evolved-iter26", "blis-evolved-iter29", "blis-evolved-iter36"])
    ...
    figures_module.SIMULATOR_DISPLAY_NAMES["blis-evolved-iter36"] = "BLIS-Evolved (Iter36)"
    # Use distinct colors: blue for iter16, orange for iter24, purple for iter26, red for iter29, teal for iter36
    ...
    figures_module.COLOR_PALETTE["blis-evolved-iter36"] = "#17BECF"  # Teal/cyan
    ...
    figures_module.HATCH_PATTERNS["blis-evolved-iter36"] = "++"

    try:
        fig = plot_simulator_comparison(
            combined,
            sim1=["blis-evolved-iter16", "blis-evolved-iter24", "blis-evolved-iter26", "blis-evolved-iter29"],
            sim2="blis-evolved-iter36",
