#!/usr/bin/env python3
"""Generate comparison plot: BLIS-Evolved Iter16 vs Iter24 vs Iter26 vs Iter27."""
...
def main():
    """Generate iter16 vs iter24 vs iter26 vs iter27 comparison figure.

    Note: This script requires results_iter16/, results_iter24/, results_iter26/, and
    results_iter27/ directories with error_records.csv files. Generate them by running:
        python -m experiment.run --adapters blis-evolved --blis-evolved-iteration {16,24,26,27}
    """
    ...
    # Combine
    combined = pd.concat([iter16_errors, iter24_errors, iter26_errors, iter27_errors], ignore_index=True)
    ...
    common_exps = iter16_exps & iter24_exps & iter26_exps & iter27_exps

    print(f"\nExperiment overlap:")
    print(f"  Common experiments: {len(common_exps)}")
    print(f"  Iter16 only: {len(iter16_exps - iter24_exps - iter26_exps - iter27_exps)}")
    print(f"  Iter24 only: {len(iter24_exps - iter16_exps - iter26_exps - iter27_exps)}")
    print(f"  Iter26 only: {len(iter26_exps - iter16_exps - iter24_exps - iter27_exps)}")
    print(f"  Iter27 only: {len(iter27_exps - iter16_exps - iter24_exps - iter26_exps)}")
    ...
    output_path = "ITER16_vs_ITER24_vs_ITER26_vs_ITER27_comparison.pdf"
    ...
    figures_module.SIMULATOR_ORDER.extend(["blis-evolved-iter16", "blis-evolved-iter24", "blis-evolved-iter26", "blis-evolved-iter27"])
    ...
    # Use distinct colors: blue for iter16, orange for iter24, purple for iter26, green for iter27
    ...
    try:
        fig = plot_simulator_comparison(
            combined,
            sim1=["blis-evolved-iter16", "blis-evolved-iter24", "blis-evolved-iter26"],
            sim2="blis-evolved-iter27",
